...
def get_contest_url_cf(path,contest_id):
    ...
    if(contest_data.find('> '+contest_id+'\n')!=-1):
        ...
        contest_data_source=get_source('https://codeforces.com/api/contest.list','cf')
        contest_data=''
        ...
        gym_data_source=get_source('https://codeforces.com/api/contest.list?gym=true','cf')
        gym_data=''
...
def get_contest_url_atc(path,contest_id):
    ...
    if(contest_data.find('> '+contest_id.lower()+'\n')!=-1):
        ...
        contest_data_source=get_source('https://atcoder.jp/contests/','atc')
        contest_data=''
        ...
        page=1
        while(True):
            contest_data_source=get_source('https://atcoder.jp/contests/archive?page='+str(page),'atc')
            source_index=contest_data_source.find('<td >')
            if(source_index==-1):
                break
