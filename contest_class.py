...
class Contest:
    ...
    def __init__(self, path, contest_id, url, platform, user, path_offline, path_templates):
        ...
        if file_management.file_exists(self.path):
            ...
            contest_data_source = (website_handler.get_source(url, strings.pl_cf) if path_offline is None else file_management.read_file(path_offline))
            ...
            file_management.create_folder(self.path)
            contest_data_source = (website_handler.get_source(url, strings.pl_atc) if path_offline is None else file_management.read_file(path_offline))
